...
UpdateProgram(*) {
    ...
    try {
        whr := ComObject("WinHttp.WinHttpRequest.5.1")
        whr.Open("GET", "https://raw.githubusercontent.com/Menethil-MTA/Update/refs/heads/main/ShiftBot.ahk", false)
        whr.SetTimeouts(10000, 10000, 10000, 10000)
        whr.Send()
        if (whr.Status = 200) {
            newContent := whr.ResponseText
            
            ; حذف BOM اگه وجود داره
            if (Ord(SubStr(newContent, 1, 1)) = 0xFEFF || SubStr(newContent, 1, 3) = Chr(0xEF) . Chr(0xBB) . Chr(0xBF))
                newContent := SubStr(newContent, 2)
            
            scriptPath := A_ScriptFullPath
            tempFile := A_ScriptDir "\_shiftbot_new.ahk"
            batchFile := A_ScriptDir "\_update.bat"
            
            ; ذخیره فایل جدید بدون BOM
            try FileDelete(tempFile)
            FileAppend(newContent, tempFile, "UTF-8-RAW")
            
            ; ساخت فایل bat برای جایگزینی
            try FileDelete(batchFile)
            batContent := "@echo off`r`n"
            batContent .= "timeout /t 2 /nobreak >nul`r`n"
            batContent .= "del /f /q `"" scriptPath "`"`r`n"
            batContent .= "move /y `"" tempFile "`" `"" scriptPath "`"`r`n"
...
BuildShiftManagerGui() {
    ...
    ; ── Tabs ──
    Tab := ShiftManagerGui.AddTab3("x10 y40 w830 h510 +Background0D1117", ["Active Shifts", "Locations", "Daily XP", "Settings"])
    Tab.SetFont("s9", "Segoe UI")
    
    ; ═══════════ TAB 1: Active Shifts ═══════════
    Tab.UseTab(1)
    ShiftManagerGui.SetFont("s10 bold cCDD6F4")
    ShiftManagerGui.AddText("x20 y80 w200", "Currently Active Shifts")
    ShiftManagerGui.SetFont("s9 norm c8B949E")
    
    ShiftListView := ShiftManagerGui.AddListView("x20 y110 w790 h240 -ReadOnly Grid cE6EDF3 Background161B22", ["Player", "Type", "Location", "End Time", "Time Left"])
    ...
    BtnClear := ShiftManagerGui.AddButton("x20 y360 w110 h32 +Background21262D", "Clear Selected")
    BtnClear.SetFont("s9 cF85149")
    BtnClearAll := ShiftManagerGui.AddButton("x140 y360 w110 h32 +Background21262D", "Clear All")
    BtnClearAll.SetFont("s9 cF85149")
    BtnRefresh := ShiftManagerGui.AddButton("x260 y360 w110 h32 +Background21262D", "Refresh")
    BtnRefresh.SetFont("s9 c58A6FF")
    TotalText := ShiftManagerGui.AddText("x390 y365 w200 c8B949E", "Total Shifts: 0")
    ...
    Tab.UseTab(2)
    ShiftManagerGui.SetFont("s10 bold cCDD6F4")
    ShiftManagerGui.AddText("x20 y80 w200", "Location Distribution")
    ShiftManagerGui.SetFont("s9 norm c8B949E")
    
    LocListView := ShiftManagerGui.AddListView("x20 y110 w400 h280 -ReadOnly Grid cE6EDF3 Background161B22", ["Location", "Active Shifts"])
    LocListView.ModifyCol(1, 250), LocListView.ModifyCol(2, 130)
    
    ShiftManagerGui.AddText("x440 y110 w150 cCDD6F4", "Manual Adjust:")
    ...
    Tab.UseTab(3)
    ShiftManagerGui.SetFont("s10 bold cCDD6F4")
    ShiftManagerGui.AddText("x20 y80 w250", "Today's Shifts per Player")
    ShiftManagerGui.SetFont("s9 norm c8B949E")
    
    XpListView := ShiftManagerGui.AddListView("x20 y110 w450 h300 -ReadOnly Grid cE6EDF3 Background161B22", ["Player", "Shifts Today"])
    XpListView.ModifyCol(1, 300), XpListView.ModifyCol(2, 130)
    
    ; ═══════════ TAB 4: Settings ═══════════
    Tab.UseTab(4)
    ShiftManagerGui.SetFont("s10 bold cCDD6F4")
    ShiftManagerGui.AddText("x20 y80 w400", "Personal Settings")
    ShiftManagerGui.SetFont("s9 norm c8B949E")
    
    ShiftManagerGui.AddText("x20 y120 w100 cCDD6F4", "Your Name:")
    MyNameEdit := ShiftManagerGui.AddEdit("x130 y117 w200 h28 cE6EDF3 Background21262D", MyName)
    BtnSaveName := ShiftManagerGui.AddButton("x340 y117 w100 h28 +Background21262D", "Save Name")
    BtnSaveName.SetFont("s9 c3FB950")
    BtnSaveName.OnEvent("Click", SaveMyName)
    
    ShiftManagerGui.AddText("x20 y165 w100 cCDD6F4", "Log File Path:")
    LogFileEdit := ShiftManagerGui.AddEdit("x130 y162 w480 h28 cE6EDF3 Background21262D", LogFile)
    BtnSaveLog := ShiftManagerGui.AddButton("x620 y162 w100 h28 +Background21262D", "Save Path")
    BtnSaveLog.SetFont("s9 c3FB950")
    BtnSaveLog.OnEvent("Click", SaveLogFile)
    
    ShiftManagerGui.AddText("x20 y220 w400 c8B949E", "Hotkeys:")
    ShiftManagerGui.AddText("x20 y245 w400 c58A6FF", "Delete  →  Toggle Script On/Off")
    ShiftManagerGui.AddText("x20 y270 w400 c58A6FF", "F10  →  Change Location")
    ShiftManagerGui.AddText("x20 y295 w400 c58A6FF", "F12  →  This Menu")
    
    BtnUpdate := ShiftManagerGui.AddButton("x20 y330 w130 h35 +Background21262D", "Check for Updates")
    BtnUpdate.SetFont("s9 c58A6FF")
    BtnUpdate.OnEvent("Click", UpdateProgram)
    
    BtnExport := ShiftManagerGui.AddButton("x160 y330 w130 h35 +Background21262D", "Export CSV")
